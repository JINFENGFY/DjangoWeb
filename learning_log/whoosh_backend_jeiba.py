...
from .tokenizer import ChineseAnalyzer
...
class WhooshSearchBackend (BaseSearchBackend):
    ...
    def build_schema(self, fields):
        schema_fields = {ID: WHOOSH_ID (stored=True, unique=True), DJANGO_CT: WHOOSH_ID (stored=True),
    ...
    def more_like_this(self, model_instance, additional_query_string=None, start_offset=0, end_offset=None, models=None,
...
class WhooshSearchQuery (BaseSearchQuery):
    ...
    def build_query_fragment(self, field, filter_type, value):
        ...
        filter_types = {'content': '%s', 'contains': '*%s*', 'endswith': "*%s", 'startswith': "%s*", 'exact': '%s',
            'gt': "{%s to}", 'gte': "[%s to]", 'lt': "{to %s}", 'lte': "[to %s]", 'fuzzy': u'%s~', }
